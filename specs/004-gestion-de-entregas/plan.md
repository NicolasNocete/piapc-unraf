...
| `activity_feedback_versions` | Historial inmutable de devoluciones: identidad, FK a entrega, numero de revision consecutivo, texto de devolucion, nota diagnostica entera de 1 a 10, `reviewer_id` con FK a `public.profiles` y `published_at`. Restriccion unica sobre `(submission_id, revision_number)`. |
