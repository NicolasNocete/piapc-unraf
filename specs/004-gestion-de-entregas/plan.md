...
| `activity_feedback_versions` | Historial inmutable de devoluciones: identidad, FK a entrega, numero de revision consecutivo, texto de devolucion, nota diagnostica entera de 1 a 10 para correcciones evaluables, indicador de material inaccesible sin nota, `reviewer_id` con FK a `public.profiles` y `published_at`. Restriccion unica sobre `(submission_id, revision_number)`. |
...
- La operacion de escritura recibe el autor o revisor solo desde el servidor, no desde `FormData`; valida que el autor sea estudiante y que el revisor sea un profesor responsable con perfil completo. Exige nota entera para correcciones evaluables y permite nota nula solo cuando el profesor marca que el material es inaccesible. La funcion atomica asigna el siguiente numero de version bajo bloqueo de la entrega para impedir colisiones entre envios concurrentes. Referencias: FR-006, FR-010, FR-012, FR-013, FR-017; AC-004, AC-006 a AC-008, AC-010.
...
3. Crear acciones de ruta con `"use server"` para enviar/reemplazar una entrega y publicar una devolucion con nota o, solo si el material es inaccesible, sin nota. Cada una debe: validar `FormData` con Zod; obtener identidad con `getClaims()`; cargar el perfil actual; derivar actividad y version desde el manifiesto por ID; comprobar que su tipo sea `actividad`; autorizar al estudiante o profesor responsable segun corresponda; y llamar el RPC atomico con valores derivados. Tratar los IDs de formulario como referencias no confiables. Las respuestas y consultas del estudiante no deben incluir la nota ni el indicador interno de inaccesibilidad. Referencias: FR-002 a FR-004, FR-006, FR-010, FR-012, FR-015, FR-017, FR-018.
...
- Actualizar el dashboard para mostrar, solo a un profesor responsable, un acceso directo a `/dashboard/entregas`, donde se presentan las actividades historicas con sus contadores de entregas, pendientes y revisadas. Referencias: FR-008, FR-009, FR-014, FR-016, FR-019.
- La ruta protegida `/dashboard/entregas` comprueba perfil responsable antes de consultar el indice de actividades. La ruta protegida `/dashboard/entregas/[activityId]` valida que el parametro corresponda a una actividad editorial vigente y presenta todas sus entregas en un dialogo accesible con texto, enlaces, versiones y formulario de devolucion con nota. Un actor no autorizado redirige al dashboard sin obtener datos. La nota no forma parte de las consultas ni props del flujo estudiantil. Referencias: FR-009 a FR-012, FR-014, FR-017 a FR-019; AC-005 a AC-008, AC-010, AC-011.
...
| FR-007 a FR-009, FR-019 | Consultas RLS por actividad, indice historico y detalle protegido | pgTAP y UI docente, AC-005, AC-011 |
| FR-010, FR-011, FR-017, FR-018 | Devoluciones y notas versionadas, con vistas separadas por actor | Pruebas de autorizacion y UI, AC-006, AC-010 |
