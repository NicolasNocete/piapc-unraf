...
export function DeliveryDetailDialog({ submissionId, activityId, studentName, versions }: Props) {
  return <Dialog><DialogTrigger render={<Button variant="outline" />}>Ver entrega</DialogTrigger><DialogContent className="max-h-[calc(100svh-2rem)] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{activityId}</DialogTitle><DialogDescription>Entrega de {studentName}. Se muestran todas las versiones registradas.</DialogDescription></DialogHeader><div className="flex flex-col gap-4">{versions.map((version) => <section key={version.version_number} className="rounded-lg border p-4"><p className="text-sm font-medium">Version {version.version_number} - {new Date(version.submitted_at).toLocaleString("es-AR")}</p><p className="mt-3 whitespace-pre-wrap break-words text-sm">{version.body}</p></section>)}</div><FeedbackForm submissionId={submissionId} /></DialogContent></Dialog>;
}
